...
async def update_picture(profile_picture:UploadFile = File(...),
                         db:Session = Depends(utils.get_db_session)
...
async def update_password(passwords : schemas.PasswordUpdate,
                          db:Session = Depends(utils.get_db_session),
                          current_user : models.User = Depends(utils.get_current_user)):
    ...
    if not user:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,detail="Please enter correct old password")
...
def delete_profile_picture( db: Session = Depends(get_db_session),
                           current_user:models.User = Depends(utils.get_current_user)):
...
def delete_phone_number( db: Session = Depends(get_db_session),
                        current_user : models.User = Depends(utils.get_current_user)):
...
async def revoke_token(token: str,current_user : models.User = Depends(utils.get_current_user)):

    
    r.set(token, "revoked", ex=30 * 60)
